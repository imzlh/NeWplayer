<template>
  <div class="user-page">
    <!-- 头部 -->
    <header class="user-header" @click="goDetailed">
      <div class="header-bg"></div>
      <div class="header-content">
        <img :src="getImageUrl(userStore.avatarUrl, 120, 120)" :alt="userStore.nickname" class="user-avatar" />
        <div class="user-info">
          <h1 class="user-name">
            {{ userStore.nickname }}
            <template v-if="userVip">
              <img v-if="userVip.redVipLevelIcon" :src="userVip.redVipLevelIcon" :alt="`VIP${userVip.redVipLevel ?? 0}`"
                class="vip-icon" />
              <span v-else class="vip-label" :class="vipTag">
                VIP{{ userVip.redVipLevel ?? 0 }}
              </span>
            </template>
          </h1>
          <p class="user-signature" v-if="userStore.user?.signature" @click="showText(userStore.user.signature)">
...
import { getLikeList, getUserEvent, getUserFollower, getUserSubcount, getUserVip } from '@/api'
import { showConfirm } from '@/stores/confirm'
import { showText } from '@/stores/text'
import { IUser, IUserCountInfo, IUserEvent, IVipInfo } from '@/api/types'
...
const userVip = shallowRef<IVipInfo>()
const vipTag = computed(() => {
  if (!userVip.value) return;
  const u = userVip.value;
  if (u.redplus || u.associator) return 'red';
  if (u.familyVip || u.musicPackage) return 'green';
  return 'black';
})
...
getUserVip().then(r => userVip.value = r.data)
...
const goToLikedSongs = () => router.push('/favorite')
const goDetailed = () => router.push('/user/' + userStore.userId)
